...
samples, samplerate = sf.read(os.path.join("test_data",'gettysburg.wav'))
output = io.BytesIO()
sf.write(output, samples, samplerate, format='WAV')
string = base64.b64encode(output.getvalue()).decode('ascii')
response = requests.post('http://localhost:5000/extract', json={'task': 'automated_speech_recognition', 'audio': string})
print(response.json())